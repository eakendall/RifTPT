...
runmodel <- function(scenario, paramvals, paramnumber=1)
{
  ...
  for (subpopulation in scenario$subpopulations)
  {
    ...
    subclinical <- future * paramvals[[paste0("subclin_per_progressor",setting,subpopulation)]]*
      paramvals[[paste0("subclin_progressor_multiplier",setting)]]*
    ...
    # - in other words, subclinical progressors who got detected would have definitely develop clinical TB if not tested, but may or may not be cured by initial treatment.
...
rownames(displaytab)[9] <- "Estimated subclinical prevalent cases per latent progressor" 
displaytab[10,] <- round(datatab["subclinical progressor ratio",],1)
rownames(displaytab)[10] <- "Estimated subclinical progressors per latent progressor" 
displaytab[9,1:4] <- NA
displaytab
write.csv(displaytab, file="Table2_cohortsubclinical.csv")

# overall prevalence subclinical progressors, PWH:
sum(datatab[3,1:4])/sum(datatab[1,1:4])
# subclinical prog/(active + subcliincal prog):
sum(datatab[3,1:4])/(sum(datatab[2:3,1:4]))

# subclinical TB, HHCs:
sum(datatab[4:5,5:7])/sum(datatab[1,5:7])
# subclinical/(active + subcliincal) [includes non-progressors]
sum(datatab[4:5,5:7])/(sum(datatab[2,5:7]) + sum(datatab[4:5,5:7]))

#uncertainty in subclinical progressors prevalence, binomially distributed, PWH:
#sd = 
sqrt(sum(datatab[1,1:4]*(datatab["subclinical progressors",1:4]*(1-datatab["subclinical progressors",1:4])/datatab[1,1:4]))/sum(datatab[1,1:4])) 
# resulting CI = 
qbinom(p=c(0.025,0.975), size = sum(datatab[1,1:4]), prob = sum(datatab[3,1:4])/sum(datatab[1,1:4]))/sum(datatab[1,1:4])

#uncertainty in subclinical progressors prevalence, binomially distributed, HHCs:
#sd of sublinical TB prevalence = 
sqrt(sum(datatab[1,5:7]*(datatab["subclinicalprevalence",5:7]*(1-datatab["subclinicalprevalence",5:7])/datatab[1,5:7]))/sum(datatab[1,5:7])) 
# combining with progression prob (sd 0.1)  [ var(𝑋)var(𝑌)+var(𝑋)𝐸(𝑌)2+var(𝑌)𝐸(𝑋)2 ]
(sd <- sqrt(
  sum(datatab[1,5:7]*(datatab["subclinical progressors",5:7]*(1-datatab["subclinical progressors",5:7])/datatab[1,5:7]))/sum(datatab[1,5:7])*(0.1)^2 + 
    sum(datatab[1,5:7]*(datatab["subclinical progressors",5:7]*(1-datatab["subclinical progressors",5:7])/datatab[1,5:7]))/sum(datatab[1,5:7])*(0.5)^2 + 
    (sum(datatab[4:5,5:7])/sum(datatab[1,5:7])/2)^2*(0.1)
    ) 
)
# CI for subclnical prevalence:
qbinom(p=c(0.025,0.975), size = sum(datatab[1,5:7]), prob = sum(datatab[4:5,5:7])/sum(datatab[1,5:7]))/sum(datatab[1,5:7])
# and for progression prob:
qbeta(p=c(0.025,0.975), 
      shape1 = AMS(mean=0.5, var=0.1^2), 
      shape2 = BMS(mean=0.5, var=0.1^2))

# For table 2 uncertainty, CIs in latent progressors by subcohort:
# (we draw parameters from beta distrbution so using that):
qbeta(p=c(0.025,0.975), 
      shape1 = AMS(mean=params$estimate[9:15], var=(params$sd[9:15])^2), 
      shape2 = BMS(mean=datatab["cumulativeprogressionincidence",], var=(datatab["sqrt variance, binomial, for cumulative incidence",])^2))

# easier to sample from empiric distributions?
colnames(sampled$params)[9:15]
colnames(sampled$params)[8]
apply(sampled$params[,9:15]*sampled$params[,8], 2, function(x) quantile(x, c(0.025,0.975)))
colnames(sampled$params)[19:22]
colnames(sampled$params)[23:25]
colnames(sampled$params)[16]
colnames(sampled$params)[17]
colnames(sampled$params)[18]
apply(sampled$params[,19:22]*sampled$params[,16]*sampled$params[,17], 2, function(x) quantile(x, c(0.025,0.975)))
apply(sampled$params[,23:25]*sampled$params[,16]*sampled$params[,18], 2, function(x) quantile(x, c(0.025,0.975)))
...
baseplotdata <- rbind(
  ...
  mutate(total_TB = subclinical_initial + latent_initial + posttreatment) %>% 
  pivot_longer(cols=c("subclinical_initial":"latent_initial", "posttreatment", "total_TB")) %>%
    mutate(Cohort = fct_relevel(Cohort, c("People with HIV", "Household contacts")),
            variable2 = fct_drop(variable),
...
tbbaseplot <- ggplot(baseplotdata %>% 
  ...
  geom_boxplot() + 
  ylab("Future incidence of any active TB, if no intervention, per cohort of 1000") + 
  xlab("") +
  theme_bw() + 
...
resbaseplot <- ggplot(baseplotdata %>% 
                     ...
                     aes(x=name3, y=value, col=name3)) +
  geom_boxplot() + 
  ylab("Future incidence of active drug-resistant TB, if no intervention, per cohort of 1000") + 
  xlab("") +
  theme_bw() + 
  theme(legend.text = element_text(size = 8),
        legend.margin =margin(0.1,0.1,0.1,2, unit='cm'),
...
gres <- ggplot(plotdata %>% filter(variable3 != "Any TB"),
  ...
  theme(legend.text = element_text(size = 8),
        legend.margin =margin(0.1,0.1,0.1,2, unit='cm'),
        axis.title = element_text(size=9)) +
  facet_grid(variable3 ~ regimen2,   space="free_y") + 
  theme(legend.position = 'bottom', legend.title=element_blank(), 
        axis.text.x=element_blank(), axis.title.x=element_blank()) + 
...
gtb <- ggplot(plotdata %>% filter(variable3 == "Any TB"),
  ...
  geom_hline(yintercept = 0, col='gray') +
  geom_boxplot(position = position_dodge(width = 0.3), outlier.size=0.4, lwd=0.5) + 
  ylab("Change in total active TB cases") + 
  theme_bw() + 
  theme(legend.text = element_text(size = 8),
        legend.margin =margin(0.1,0.1,0.1,2, unit='cm'),
        axis.title = element_text(size=9)) +
...
K0$alllong %>% filter(variable=="allRR") %>% group_by(paramnumber) %>% 
  ...
  summarize(quantile(proportion, probs = c(0.5,0.025,0.975)),
            quantile(number, probs = c(0.5,0.025,0.975)))
K0$alllong %>% filter(variable %in% c("RR","allRR")) %>% group_by(paramnumber) %>% 
  pivot_wider(names_from = c("initialTB", "casetiming")) %>% 
  mutate(all = subclinical_initial + latent_initial + subclinical_posttreatment + latent_posttreatment ) %>% 
  select(-c(subclinical_initial, latent_initial, subclinical_posttreatment, latent_posttreatment )) %>% 
  pivot_longer(all) %>% 
...
# RR added per TB, HR, and MDR averted, 4R vs placebo
...
# Hmono added per TB pr3vented, 6H vs none
...
# format like figure
# columns: 4R sxs, 6H sxs, 4R cxr, 6H cxr, 
...
rownames(resulttable) <- c(
  ...
  "Symptomatic TB cases added (vs no TPT)",
...
# Supplemental figure comparing 4R vs 6H head to head

getregdiffs <- function(alllong)
{ return(
    alllong %>% filter(variable %in% c("TB","allRR","HR")) %>% 
    pivot_wider(names_from = regimen, values_from=value) %>%
    mutate(regdiff = `4R` - `6H`) %>% 
    select(-none, -`4R`,-`6H`) %>% 
    pivot_longer(cols = contains('diff'), names_to = 'regimen' ) %>% 
    pivot_wider(names_from=c(initialTB, casetiming), values_from = c(value)) %>% 
    mutate(posttreatment = latent_posttreatment + subclinical_posttreatment) %>% 
...
getregdiffs(K0$alllong)

regplotdata <-
  rbind(getregdiffs(K80$alllong) %>% mutate(screen=TRUE, cohort="PWH"), 
        getregdiffs(K0$alllong) %>% mutate(screen=FALSE, cohort="PWH"),
        getregdiffs(P80$alllong) %>% mutate(screen=TRUE, cohort="HHCs"), 
        getregdiffs(P0$alllong) %>% mutate(screen=FALSE, cohort="HHCs")) %>% 
  mutate(name2 = fct_relevel(name, c("latent_initial","subclinical_initial",
                                     "posttreatment","total_TB")),
         name3 = fct_recode(name2, "From latent" = "latent_initial",
                            "From subclinical"="subclinical_initial", 
         ...
         name5 = fct_relevel(name4,
                             c("From latent", " ",
                               ...
                               "After future treatment", "   ",
                               "All TB combined", "    ")),
         cohort = fct_relevel(cohort, c("PWH","HHCs")))


gres <- ggplot(regplotdata %>% filter(variable3 != "Any TB"),
               aes(x=name3, y=value, col=name5)) + 
  geom_hline(yintercept = 0, col='gray') +
  geom_boxplot(position = position_dodge(width = 0.3), outlier.size=0.4, lwd=0.5) + 
  ylab("Change in drug-resistant TB cases") + 
  ...
  theme(legend.text = element_text(size = 8),
        legend.margin =margin(0.1,0.1,0.1,2, unit='cm'),
        axis.title = element_text(size=9)) +
  facet_wrap(variable3 ~ cohort) + 
  theme(legend.position = 'bottom', legend.title=element_blank(), 
        axis.text.x=element_blank(), axis.title.x=element_blank()) + 
  scale_colour_manual(values=hcl(rep(seq(15,300,length.out=4),each=2), 
                                 c = 100 , l = 65+ rep(c(-15,15), times=4))) 

gtb <- ggplot(regplotdata %>% filter(variable3 == "Any TB"),
              aes(x=name3, y=value, col=name5)) + 
  geom_hline(yintercept = 0, col='gray') +
  geom_boxplot(position = position_dodge(width = 0.3), outlier.size=0.4, lwd=0.5) + 
  ylab("Change in total active TB cases") + 
...
pdf(file="TPT model Fig S2 regimen diffs.pdf", width = 9, height=6)
diffFig
dev.off()

##### Part 3: symptom-only versus more sensitive screening, still uptake = 1-noncompletion ###########
K50$alllong %>% filter(variable=="TB") %>% group_by(paramnumber) %>% 
  pivot_wider(names_from = "regimen") %>%
  summarize(proportion=(sum(none)-sum(`4R`))/sum(none), number=sum(none)-sum(`4R`)) %>% 
  summarize(quantile(proportion, probs = c(0.5,0.025,0.975)),
            quantile(number, probs = c(0.5,0.025,0.975)))
...
save(allout, file=paste0("TPT varying uptake",setting,".Rdata"))
...
grangetb <- ggplot(fig4data %>% filter(variable =="TB"),
  ...
  ylab("Change in active TB cases") + 
  xlab("Reduction in TPT access with subclinical screening requirement") +
  theme_bw() + 
  ...
  geom_text(data = nout_summary %>% filter(variable2=="Any TB"), aes(y = med, x="60%"), 
            ...
            vjust=1.5, nudge_y = 0.3, hjust=0.5, size=3, 
            col=hcl(rep(seq(15,300,length.out=4),each=2), 
                        c = 100 , l = 65+ rep(c(-10,10), times=4))[7])

ylim <- ifelse(setting=="_pak",3,8)
...
# # now plot comparing high and low deciles
...
######## Under 5 contacts only ##########
params[1:7,1:3]
sampled5 <- sampled; sampled5$params[,6:7]<-0
P80_lt5 <- getresults("_pak",uptake=0.8, sens=0.9,  sampled=sampled5)
P0_lt5 <- getresults("_pak",uptake=1, sens=0,  sampled=sampled5) # no screening

setting <- "_pak"
withscreening <- P80_lt5$diffs
noscreening <- P0_lt5$diffs
...
pdf(file="TPT model under5 diffs.pdf", width = 9, height=6)
diffFig
dev.off()
